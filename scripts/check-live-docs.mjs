...
export function parseDiscoveryLinks(content, indexUrl = DOCS_INDEX_URL) {
  ...
  const links = [...content.matchAll(/\[[^\]]+\]\((?<url>https:\/\/[^)\s]+|\/[^)\s]+)\)/gu)].map(
    (match) => new URL(match.groups.url, indexUrl),
  );
  const guideUrls = [];
  ...
  if (declaredOpenApiUrls.some((url) => url.href !== OPENAPI_URL)) {
    throw new Error("llms.txt references an unexpected OpenAPI artifact.");
  }
...
async function fetchBoundedText(url, fetchImpl) {
  ...
  for (let redirects = 0; redirects <= 3; redirects += 1) {
    ...
    if (declaredLength > maximumResponseBytes) throw new Error(`${currentUrl} exceeded the response-size limit.`);
...
export async function checkLiveDocs({ fetchImpl = globalThis.fetch } = {}) {
  ...
  for (const guideUrl of discovery.guideUrls) {
    ...
    if (!guide.includes(OPENAPI_URL)) throw new Error(`${guideUrl} does not link the canonical OpenAPI artifact.`);
  }
